...
public class EstadoPesquisa implements Serializable {
	...
	public List<Estado> porPais(Pais pais) {
		Session sessaoDoHibernate = this.gerenciadorEntidade.unwrap(Session.class);
		Criteria resultado = sessaoDoHibernate.createCriteria(Estado.class);
		try {
			if (pais != null) {
				return resultado.add(Restrictions.eq("pais", pais)).list();
			} else {
				return null;
			}
	...
	private Criteria criarCriteriaParaFiltro(EstadoFiltro filtro) {
		Session sessaoHibernate = this.gerenciadorEntidade.unwrap(Session.class);
		Criteria resultado = sessaoHibernate.createCriteria(Estado.class);
